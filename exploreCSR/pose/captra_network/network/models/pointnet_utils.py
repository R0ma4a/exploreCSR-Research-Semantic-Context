...
def knn_point(k, pos2, pos1):
    ...
    '''
    if CUDA:
        val, idx = futils.knn(k, pos2.contiguous(), pos1.contiguous())
        return val, idx.long()
...
def three_nn(xyz1, xyz2):
    if CUDA:
        dists, idx = futils.three_nn(xyz1.contiguous(), xyz2.contiguous())
...
def farthest_point_sample(xyz, npoint):
    ...
    if CUDA:
        idx = futils.furthest_point_sample(xyz.contiguous(), npoint).long()
        return idx
...
def query_ball_point(radius, nsample, xyz, new_xyz):
    ...
    _, S, _ = new_xyz.shape
    if CUDA:
        return futils.ball_query(radius, nsample, xyz.contiguous(), new_xyz.contiguous()).long()
...
class PointNetSetAbstractionMsg(nn.Module):
    ...
    def forward(self, xyz, points):
        ...
        B, C, N = xyz.shape
        S = self.npoint
        fps_idx = farthest_point_sample(xyz.permute(0, 2, 1).contiguous(), S)
