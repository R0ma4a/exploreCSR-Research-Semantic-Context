...
class SequenceData(PointData):
    def __init__(self, cfg, mode='train', downsampling=None):
        ...
        if 'bmvc' in self.mode:
            self.len = 1
        elif self.nocs_data or self.real_data:
            # normalize Windows backslashes before grouping
            self.dataset.file_list = [f.replace('\\', '/') for f in self.dataset.file_list]
            ...
            if len(self.dataset) >= self.num_frames:
                remainder = len(self.dataset) % self.num_frames
                if remainder != 0:
                    self.dataset.file_list = self.dataset.file_list[:-remainder]
                    print(f"Trimmed dataset to {len(self.dataset)} frames (removed {remainder} incomplete frames)")
            else:
...
def get_dataloader(cfg, mode='train', shuffle=None, downsampling=None):
    ...
    if 'track' in cfg['network']['type']:
        ...
        if dataset.nocs_data or dataset.real_data:
            batch_size = 1
    else:
